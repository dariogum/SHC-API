...
class Patient {
	...
	private function add(Request $request, Response $response, $args) {
		...
		$patientId = $this->table->insertGetId($body['data']['attributes']);
	...
	private function delete(Request $request, Response $response, $args) {
		...
		if (array_key_exists("id", $args)) {
			...
			$patient = $this->table->find($args['id']);

			if ($patient) {
				$visitsQuery = $this->visitsTable->where('patient', $args['id'])->delete();
				$result = $this->table->where('id', $args['id'])->delete();
			} else {
				$errors = [
					"errors" => [
						"id" => "404",
	...
	private function getVisitFiles($visitId) {
		$filesData = [];

		$filesQueryBuilder = clone $this->filesTable;

		$files = $filesQueryBuilder->where('visit', $visitId)->get();
	...
	private function getVisits($patientId, $withVisitsFiles) {
		$visitsData = [];

		$visitsQueryBuilder = clone $this->visitsTable;

		$visits = $visitsQueryBuilder->where('patient', $patientId)->latest('date')->get();
		...
		foreach ($visits as $visit) {
			...
			if ($withVisitsFiles) {
				$visitData["data"]["relationships"]["files"] = $this->getVisitFiles($visit->id);
			}
	...
	private function patientToJson($patient, $withVisits = true, $withVisitsFiles = true) {
		...
		if ($withVisits) {
			$jsonPatient["relationships"]["visits"] = $this->getVisits($patient->id, $withVisitsFiles);
		}
